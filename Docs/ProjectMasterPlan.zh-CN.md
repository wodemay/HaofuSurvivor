...
| 存档 | 继续游戏、玩家和地图状态恢复 | 根目录 JSON 快照，Model 数据优先，View 运行时重建 | 已接入，需长局回归；防篡改为可选增强 | Run、Player、Enemy、Map、Progression |
...
1. 读取并校验快照版本与内容。
2. 清理现有运行时，按保存角色生成 PlayerRoot，并启动对局基础系统。
3. 恢复玩家位置、地图身份、区块障碍和地图事件数据。
4. 恢复对局时间、经济、生命、经验和属性状态。
5. 重建 Weapon、Skill、Dodge 与升级选择状态。
6. 恢复敌人、经验球、投射物、持续效果、道具和攻击冷却。
7. 恢复随机状态、生命表现和保存时的对局阶段。
8. 任一步骤失败都清理本次生成的运行时对象并返回主菜单。
...
1. 存档完整性、版本迁移、恢复和结算失败闭环已完成；下一步补长时间整局继续与结算回归。
2. 以幸存者为样板完成 M1/M2 的全流程回归闭环。
