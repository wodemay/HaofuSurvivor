...
- Projectile attack ID 1002 uses executor ID `projectile`, `ProjectileAttackParameterConfig`, and the user-created `Art/Prefabs/Attack/Bullet.prefab`. It finds the nearest enemy through the CombatTargetSystem query and returns instances to ProjectileFactory's runtime pool. Keep base values in the parameter asset; future upgrades apply runtime modifiers by attack ID.
- CharacterConfig.SkillGroupId resolves through SkillGroupCatalog. A SkillGroupConfig defines starting weapon IDs, skill IDs, and one dodge ID; WeaponConfig maps a weapon to AttackIds. PlayerLoadoutSystem owns the runtime loadout and currently equips only weapons. Future upgrades must call it rather than modify ScriptableObject assets.
