...
- Configure all actor attacks through integer attack IDs and `AttackCatalog` in `Resources/Configs/Combat/Attack/`. `AttackConfig.ExecutorId` selects a registered executor; never add an attack-type enum or central type branch. `AttackConfig` does not define a target faction: standard attacks derive the opposing faction from their runtime CombatEntity owner. The implemented collision attack uses ID 1001 and executor ID `collision`.
- Projectile attack ID 1002 uses executor ID `projectile`, `ProjectileAttackParameterConfig`, and the user-created `Art/Prefabs/Attack/Bullet.prefab`. It is an automatic attack: `ConfigureOwner` calls `AttackSystem.RegisterAutomatic`, and `IAutomaticAttackExecutor.FindTarget` resolves the nearest enemy through the CombatTargetSystem query on the shared `GameLoopSystem` tick — there is no per-attack Update or trigger component. Instances return to ProjectileFactory's runtime pool, which must skip destroyed pooled references after scene changes. Keep base values in the parameter asset; future upgrades apply runtime modifiers by attack ID.
...
- `CollisionAttackTrigger` registers an AttackRuntime and uses generic CombatEntity faction overlap. EnemyRoot and PlayerRoot receive CombatEntity at runtime; EnemyRoot also receives a Kinematic Rigidbody2D for trigger callbacks.
- Player attack input is implemented on the shared AttackSystem: equipped weapons auto-fire through `AttackSystem.RegisterAutomatic` + `IAutomaticAttackExecutor.FindTarget`, `Space` sends `RequestSkillCommand` for the equipped manual skill, and `Left/Right Shift` sends `RequestDodgeCommand` for the equipped dodge. All four executors (`collision`, `projectile`, `explosive-projectile`, `barrage-projectile`) are registered in `AttackExecutorRegistry`.
